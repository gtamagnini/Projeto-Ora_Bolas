var robo_raio = 0.09; // diametro do small size é 180mm, sendo 90mm de raio, transformando em m = 0.09
var v_max = 2.8; // velocidade máxima do robô
var Tf = 0;
var i = 0;

// tempo obtido do arquivo time.txt
...
function vetor() {
    ...
    for (i = 0; i<1002; i++){
        v_bolax[i] = (-0.005*tempo[i]) + 0.5;
    }

    // velocidade da bola no eixo y
    for (i = 0; i<1002; i++){
        v_bolay[i] = (-0.008*tempo[i]) + 0.4;
    }
    // aceleração do robô
    for (i = 0; i<1002; i++){
        v_robo[i] = a_robo[i] * tempo[i];

        // caso a velocidade for máxima, a aceleração será igual a 0
        if(v_robo[i] >= v_max){
            v_robo[i] = v_max;
            a_robo[i] = 0;
        }
    ...
    for (i=0;i<1002;i++){
        distancia[i] = parseFloat(Math.sqrt((Math.pow((x_bola[i] - robo_x[i]), 2) + Math.pow((y_bola[i] - robo_y[i]), 2))));

        cos[i] = parseFloat((x_bola[i] - robo_x[i]) / distancia[i]);
        sen[i] = parseFloat((y_bola[i] - robo_y[i]) / distancia[i]);


        robo_x[i + 1] = parseFloat(robo_x[i] + (cos[i] * 0.04)); //vai aumentando a posicao do robo em X
        robo_y[i + 1] = parseFloat(robo_y[i] + (sen[i] * 0.05)); //vai aumentando a posicao do robo em Y


        // calculo do tempo final
        if (robo_x[i] >= x_bola[i] - robo_raio && robo_x[i] <= x_bola[i] + robo_raio && robo_y[i] >= y_bola[i] - robo_raio && robo_y[i] <= y_bola[i] + robo_raio) {
            Tf = tempo[i];

            for (i=0;i<1002;i++){
                tempo_final[i] = tempo[i];
                if(tempo_final[i] == Tf){
                    break;
                }
            }
            alert("Tempo Final do percurso: " + Tf);
            break;
        }
    }
    grafico();
...
function grafico(){
    // TRAJETÓRIAS DA BOLA E DO ROBÔ EM UM PLANO X E Y, ATÉ O PONTO DE INTERCEPTAÇÃO
    ...
    var data = [traj_robo, traj_bola];

    var layout = {
        title:"TRAJETÓRIA DA BOLA E DO ROBÔ"
    };
    Plotly.newPlot("grafico_t", data, layout);

//COORDENADAS  X E Y DA POSIÇÃO DO ROBÔ E DA BOLA EM FUNÇÃO DO TEMPO T ATÉ O INSTANTE DA INTERCEPTAÇÃO
    ...
    var data = [posicao_robox, posicao_bolax];
    var layout = {
        title: "POSIÇÃO DA BOLA E DO ROBÔ NO EIXO X EM FUNÇÃO DO TEMPO"
    };
    Plotly.newPlot("grafico_px", data, layout);
    ...
    var data = [posicao_roboy, posicao_bolay];
    var layout = {
        title: "POSIÇÃO DA BOLA E DO ROBÔ NO EIXO Y EM FUNÇÃO DO TEMPO"
    };
    Plotly.newPlot("grafico_py", data, layout);

//COMPONENTES VX E VY DA VELOCIDADE DA BOLA E DO ROBÔ EM FUNÇÃO DO TEMPO T ATÉ O INSTANTE DE INTERCEPTAÇÃO

    var velo_robox = {
        x: tempo_final, //SE NAO FOR ASSIM, TROCAR PARA vetTemp ==========
        y: v_robo,
        mode: 'lines+markers',
        name: '"ROBÔ"',
        line: {
    ...
    var data = [velo_robox,velo_bolax];
    var layout = {
        title: "VELOCIDADE DA BOLA E DO ROBÔ NO EIXO X"
    };
    Plotly.newPlot("grafico_vx", data, layout);
    ...
    var data = [velo_roboy,velo_bolay];
    var layout = {
        title: "VELOCIDADE DA BOLA E DO ROBÔ NO EIXO Y"
    };
    Plotly.newPlot("grafico_vy", data, layout);

//COMPONENTES AX E AY DA VELOCIDADE DA BOLA E DO ROBÔ EM FUNÇÃO DO TEMPO T ATÉ O INSTANTE DE INTERCEPTAÇÃO
    ...
    var data = [a_robox, a_x, a_y];
    var layout = {
        title: 'X e Y do robo e bola pelo tempo'
    };
    Plotly.newPlot("grafico_a", data, layout);

//DISTÂNCIA RELATIVA D ENTRE O ROBÔ E A BOLA COMO FUNÇÃO DO TEMPO ATÉ O INSTANTE DE INTERCEPTAÇÃO

    var distancia_robo = {
        x: tempo_final,
        y: distancia,
        mode: 'lines+markers',
        name: 'DISTÂNCIA RELATIVA',
        line: {
            color: "#00FF00",
            width: 2
        }
    ...
    var data = [distancia_robo];
    var layout = {
        title: "DISTÂNCIA RELATIVA ENTRE O ROBÔ E A BOLA"
    };
    Plotly.newPlot("grafico_d", data, layout);
}
